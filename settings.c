...
#elif N_AXIS == 5
    {Settings_Axis_Rotational, Group_Stepper, "Rotational axes", NULL, Format_Bitfield, "A-Axis,C-Axis", NULL, NULL, Setting_IsExtendedFn, set_rotational_axes, get_int, NULL},
#elif N_AXIS == 6
...
static uint32_t get_int(setting_id_t id)
{
    ...
    case Setting_HardLimitsEnable:
        value = ((settings.limits.flags.hard_enabled & bit(0)) ? bit(0) |
                                                                     ...
                                                                     (settings.limits.flags.hard_disabled_rotary ? bit(2) : 0)
                                                               : 0);
        break;
...
static bool settings_read_tool_data(tool_id_t tool_id, tool_data_t *tool_data)
{
    ...
    if (!(hal.nvs.type != NVS_None && hal.nvs.memcpy_from_nvs((uint8_t *)tool_data, NVS_ADDR_TOOL_TABLE + (tool_id - 1) * (sizeof(tool_data_t) + NVS_CRC_BYTES), sizeof(tool_data_t), true) == NVS_TransferResult_OK && tool_data->tool_id == tool_id))
    {
